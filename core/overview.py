...
数据源全部来自实时主表（cb_exchange_order / cb_user / cb_site / cb_bike /
cb_battery / cb_exchange / cb_expense_bill），不使用停更的统计表。
"""
import time
from core import db
...
def summary(oem_ids=None, args=None):
    ...
    t = db.query(
        ...
        f"FROM cb_exchange_order o WHERE {where} AND o.create_time>{today} AND o.create_time<={today_ms}")[0]
    # 昨日换电
    y = db.query(
        f"SELECT COUNT(*) cnt FROM cb_exchange_order o "
        f"WHERE {where} AND o.create_time>{today - DAY_MS} AND o.create_time<={today}")[0]
    # 用户累计
    u = db.query(f"SELECT COUNT(*) cnt FROM cb_user u WHERE {w2}")[0]
    ...
    ej = " LEFT JOIN cb_site s ON s.id=e.site_id AND s.is_del=0 " if (ef and "s." in ef.and_clause()) else ""
    dev = db.query(
        "SELECT "
        f"(SELECT COUNT(*) FROM cb_exchange e {ej} WHERE {ew}) ex, "
        f"(SELECT COUNT(*) FROM cb_exchange e {ej} WHERE {ew} AND e.online_status='online') ex_on, "
        f"(SELECT COUNT(*) FROM cb_battery b WHERE {bw}) bt, "
        f"(SELECT COUNT(*) FROM cb_battery b WHERE {bw} AND b.online_status='online') bt_on, "
        f"(SELECT COUNT(*) FROM cb_bike WHERE is_del=0) bk, "
        f"(SELECT COUNT(*) FROM cb_site s WHERE {sw}) st")[0]

    # 今日新增用户
    nu = db.query(f"SELECT COUNT(*) c FROM cb_user u WHERE {w2} AND u.create_time>{today} AND u.create_time<={today_ms}")[0]
    # 活跃用户（今日有换电）
    return {
...
def monthly_trend(oem_ids=None, months=24, args=None):
    ...
    rows = db.query(
        ...
        f"FROM cb_exchange_order o WHERE {where} AND o.create_time>{start} "
        f"GROUP BY ym ORDER BY ym")
    # 收入口径：分成收入取 profit_fee（毛利）
    rows2 = db.query(
        f"SELECT DATE_FORMAT(FROM_UNIXTIME(o.create_time/1000), '%%Y-%%m') ym, "
        f"COALESCE(SUM(o.profit_fee),0) profit "
        f"FROM cb_exchange_order o WHERE {where} AND o.create_time>{start} AND o.profit_fee>0 "
        f"GROUP BY ym ORDER BY ym")
...
def city_dimension(oem_ids=None, days=30, args=None):
    """城市维度：按 sys_city_name 聚合（近 N 天换电 + 累计用户/设备；支持订单维度筛选）"""
    where = "o.is_del=0"
    if oem_ids:
        where += f" AND o.oem_id IN ({','.join(map(str, oem_ids))})"
    of = _flt(args, ORDER_COLUMNS)
    ...
    rows = db.query(
        f"SELECT COALESCE(NULLIF(o.sys_city_name,''),'未知城市') city, "
        f"COUNT(*) cnt, COUNT(DISTINCT o.take_user_id) users, "
...
def regions(oem_ids=None, days=30, args=None):
    ...
    # 用户按 city 字段（cb_user.city 是城市名，部分为空）
    uw = "u.is_del=0"
    if oem_ids:
    ...
    if uf:
        uw += uf.and_clause()
    urows = db.query(f"SELECT COALESCE(NULLIF(u.city,''),'未知') city, COUNT(*) c FROM cb_user u WHERE {uw} GROUP BY city")
    umap = {_city_core(r["city"]): int(r["c"]) for r in urows}

    # 换电柜按网点城市分组（cb_exchange.site_id -> cb_site.city；无网点柜子归未知）
    exw = "e.is_del=0"
    if oem_ids:
    ...
    exrows = db.query(
        ...
        f"FROM cb_exchange e LEFT JOIN cb_site s ON s.id=e.site_id AND s.is_del=0 WHERE {exw} GROUP BY city")
    ...
    orows = db.query(
        ...
        f"FROM cb_exchange_order o WHERE {ow} AND o.create_time>{start} GROUP BY city")
...
def device_dimension(oem_ids=None, args=None):
    ...
    bt = db.query(
        ...
        f"FROM cb_battery b WHERE {bt_w}")[0]
    rows = db.query(
        "SELECT "
        "(SELECT COUNT(*) FROM cb_bike WHERE is_del=0) bk_total, "
        "(SELECT COUNT(*) FROM cb_bike WHERE is_del=0 AND online_status='online') bk_on, "
        "(SELECT COUNT(*) FROM cb_bike WHERE is_del=0 AND online_status='offline') bk_off, "
        "(SELECT COUNT(*) FROM cb_site WHERE is_del=0) st_total, "
        "(SELECT COUNT(*) FROM cb_site WHERE is_del=0 AND site_status='on') st_open, "
        "(SELECT COUNT(*) FROM cb_site WHERE is_del=0 AND site_status='off') st_closed")[0]
    return {
...
def finance_dimension(oem_ids=None, months=12, args=None):
    ...
    income = db.query(
        f"SELECT DATE_FORMAT(FROM_UNIXTIME(o.create_time/1000), '%%Y-%%m') ym, "
        f"COALESCE(SUM(o.real_pay_price),0)+COALESCE(SUM(o.expend_power_fee),0) fee, COALESCE(SUM(o.profit_fee),0) profit "
        f"FROM cb_exchange_order o WHERE {iw} AND o.create_time>{start} GROUP BY ym ORDER BY ym")
    expense = db.query(
        f"SELECT DATE_FORMAT(FROM_UNIXTIME(b.create_time/1000), '%%Y-%%m') ym, "
        f"COALESCE(SUM(b.fee),0) fee, COUNT(*) cnt "
        f"FROM cb_expense_bill b WHERE {ew} AND b.create_time>{start} GROUP BY ym ORDER BY ym")
