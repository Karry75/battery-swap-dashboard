...
def city_options(oem_ids=None):
    """城市列表：cb_site.city + cb_exchange_order.site_city 去重合并"""
    out = {}
    _o = _oem_in(oem_ids)
    for r in _rows(f"SELECT DISTINCT city FROM cb_site WHERE is_del=0 AND city IS NOT NULL AND city<>''{_o}"):
        if r.get("city"):
            out[r["city"]] = r["city"]
    for r in _rows(f"SELECT DISTINCT site_city FROM cb_exchange_order WHERE is_del=0 AND site_city IS NOT NULL AND site_city<>''{_o} LIMIT 2000"):
        if r.get("site_city"):
...
def street_options(oem_ids=None):
    ...
    rows = _rows(f"SELECT DISTINCT street FROM cb_site WHERE {w} LIMIT 1000")
    out = {r["street"] for r in rows if r.get("street")}
    return [{"value": k, "label": k} for k in sorted(out)]


def community_options(oem_ids=None):
    """社区列表：cb_site.community"""
    w = "is_del=0 AND community IS NOT NULL AND community<>''" + _oem_in(oem_ids)
    rows = _rows(f"SELECT DISTINCT community FROM cb_site WHERE {w} LIMIT 1000")
    out = {r["community"] for r in rows if r.get("community")}
    return [{"value": k, "label": k} for k in sorted(out)]


def agency_options(oem_ids=None):
    """代理商列表：cb_exchange_order.agency_id/agency_name + cb_site.agency_id"""
    out = {}
    _o = _oem_in(oem_ids)
    for r in _rows(f"SELECT DISTINCT agency_id, agency_name FROM cb_exchange_order "
...
def battery_product_options(oem_ids=None):
    """电池产品列表：cb_battery_product"""
    w = "is_del=0" + _oem_in(oem_ids)
    rows = _rows(f"SELECT id, name, code FROM cb_battery_product WHERE {w} LIMIT 1000")
...
def battery_brand_options(oem_ids=None):
    """电池品牌列表：cb_battery_brand"""
    w = "is_del=0" + _oem_in(oem_ids)
    rows = _rows(f"SELECT id, name FROM cb_battery_brand WHERE {w} LIMIT 1000")
    return [{"value": str(r["id"]), "label": (r.get("name") or f"#{r['id']}")}
            for r in rows if r.get("id") is not None]


def bus_unit_options(oem_ids=None):
    """事业线列表：cb_business_unit"""
    w = "is_del=0" + _oem_in(oem_ids)
    rows = _rows(f"SELECT id, name FROM cb_business_unit WHERE {w} LIMIT 1000")
    return [{"value": str(r["id"]), "label": (r.get("name") or f"#{r['id']}")}
            for r in rows if r.get("id") is not None]


def site_options(oem_ids=None, keyword=""):
    """网点列表：cb_site（量大约束 2000 条）"""
    w = "is_del=0" + _oem_in(oem_ids)
    if keyword:
        w += f" AND name LIKE '%%{keyword}%%'"
    rows = _rows(f"SELECT id, name, city, area, address FROM cb_site WHERE {w} "
...
def employee_options(oem_ids=None):
    """门店业务员列表：cb_site_store_employee"""
    w = "is_del=0" + _oem_in(oem_ids)
    rows = _rows(f"SELECT id, name, serve_site_id, serve_site_name, merchant_id "
                 f"FROM cb_site_store_employee WHERE {w} LIMIT 2000")
...
def merchant_options(oem_ids=None):
    """商户列表：cb_merchant"""
    w = "is_del=0" + _oem_in(oem_ids)
    rows = _rows(f"SELECT id, name, merchant_id FROM cb_merchant WHERE {w} LIMIT 2000")
